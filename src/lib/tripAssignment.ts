...
export async function claimTripResources(tx: Tx, input: TripResourceInput): Promise<TripResourceResult> {
  ...
  const truck = await tx.truck.findUnique({ where: { id: input.truckId }, include: { plant: true } });
  ...
  if (truck.plant.siteId !== input.siteId) return { status: "TRUCK_OUT_OF_SCOPE" };
  ...
  const pump = await tx.pump.findUnique({ where: { id: input.pumpId }, include: { plant: true } });
  ...
  if (pump.plant.siteId !== input.siteId) return { status: "PUMP_OUT_OF_SCOPE" };
  if (pump.reachM != null && input.minPumpReachM != null && pump.reachM < input.minPumpReachM) return { status: "PUMP_INSUFFICIENT_REACH" };
  ...
  if (crewIds.length > 0) {
    ...
    if (input.pumpOperatorId) {
      const op = await tx.pumpCrewMember.findUnique({ where: { id: input.pumpOperatorId } });
      if (!op || op.role !== "OPERATOR" || op.status !== "ACTIVE") return { status: "PUMP_OPERATOR_INVALID" };
      pumpOperatorName = op.name;
    }
...
export async function reassignTrip(
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      const trip = await tx.trip.findUniqueOrThrow({ where: { id: tripId }, include: { batchTicket: { include: { reservation: true, plant: true } } } });
      ...
      const resources = await claimTripResources(tx, {
        siteId: trip.batchTicket.plant.siteId,

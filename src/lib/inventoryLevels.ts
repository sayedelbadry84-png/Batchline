...
// old JS code had) entirely inside one SQL UPDATE via GREATEST(0, level +
// delta) — the database computes the new value from whatever the row
// currently holds at the instant it runs, not from a value read earlier
// in application code, so the race above can't happen regardless of
// transaction isolation level. delta is negative for a deduction,
// positive for a credit; the resulting level is returned via RETURNING so
// callers that need it (auto-requisition threshold checks) don't need a
// second read.
...
export async function adjustSiloLevel(db: Db, siloId: string, deltaTons: number): Promise<number | null> {
  const rows = await db.$queryRaw<{ currentLevelTons: number }[]>`
    UPDATE "Silo" SET "currentLevelTons" = GREATEST(0, "currentLevelTons" + ${deltaTons})
...
export async function adjustHopperLevel(db: Db, hopperId: string, deltaTons: number): Promise<number | null> {
  const rows = await db.$queryRaw<{ currentLevelTons: number }[]>`
    UPDATE "Hopper" SET "currentLevelTons" = GREATEST(0, "currentLevelTons" + ${deltaTons})
...
export async function adjustChemicalTankLevel(db: Db, tankId: string, deltaLiters: number): Promise<number | null> {
  const rows = await db.$queryRaw<{ currentLevelLiters: number }[]>`
    UPDATE "ChemicalTank" SET "currentLevelLiters" = GREATEST(0, "currentLevelLiters" + ${deltaLiters})
